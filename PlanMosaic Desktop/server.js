...
- "把X改成Y"/"X换成Y" → update_task 或 modify_schedule(operation="modify_slot")
...
async function callDeepseekAI(messages, scheduleData, retryCount = 0, depth = 0, overrideModel = null, _pendingProposal = null) {
    const modelName = overrideModel || MODEL_NAME;
    // 检查API密钥是否已配置
    if (!DEEPSEEK_API_KEY || DEEPSEEK_API_KEY === 'YOUR_DEEPSEEK_API_KEY_HERE') {
    ...
    try {
        ...
        if (result.choices[0]?.message?.tool_calls && result.choices[0]?.message?.tool_calls.length > 0) {
            ...
            // 从工具结果中提取 proposal
            let pendingProposal = _pendingProposal;
            for (const tr of toolResults) {
                try {
                    const trObj = JSON.parse(tr.result);
                    if (trObj.proposal) {
                        pendingProposal = trObj.proposal;
                        break;
                    }
                } catch (e) {
                    // 不是 JSON，跳过
            ...
            const followUpResult = await callDeepseekAI(newMessages, scheduleData, retryCount, depth + 1, overrideModel, pendingProposal);
        ...
        if (error.message && (error.message.includes('HTTP 5') || error.message.includes('HTTP 429')) && retryCount < maxRetries) {
            ...
            await sleep(retryDelay * (retryCount + 1));
            return callDeepseekAI(messages, scheduleData, retryCount + 1, depth, overrideModel, _pendingProposal);
        }
        ...
        // 返回降级响应
        return getFallbackResponse(error, messages, scheduleData, _pendingProposal);
    }
}
...
// 降级响应：当AI调用失败时使用
function getFallbackResponse(error, messages, scheduleData, _pendingProposal = null) {
...
// ============ ReAct 转录生成函数 ============
function generateReActLog(messages, full = false) {
    let reactLog = '';
            ...
            for (const tc of msg.tool_calls) {
                ...
                for (let j = i + 1; j < messages.length; j++) {
                    if (messages[j].role === 'tool' && messages[j].tool_call_id === toolCallId) {
                        ...
                        const truncated = full ? result : (result.length > 300 ? result.substring(0, 300) + '...(截断)' : result);
    ...
    if (full && reactLog) {
        ...
        const header = `${sep}\n` +
            `生成时间: ${datetime}\n` +
            `对话消息总数: ${messages.length}\n` +
            `格式: ReAct (Reasoning + Acting)\n` +
            `说明: 由 PlanMosaic AI Agent 自动生成，基于原生 OpenAI Function Calling 转录\n` +
            `${sep}\n\n`;
        reactLog = header + reactLog;
    }
...
async function executeSingleToolCall(toolCall, scheduleData) {
    ...
    switch (routedName) {
        ...
        case 'modify_schedule': {
            ...
            return JSON.stringify({type:'proposal',date:d,changes:routedArgs.changes||[],reason:routedArgs.reason,timeSlots:routedArgs.timeSlots,newSlotDetails:routedArgs.newSlotDetails,proposal:{type:'modify_schedule',operation:op,date:d,changes:routedArgs.changes,reason:routedArgs.reason,timeSlots:routedArgs.timeSlots,newSlotDetails:routedArgs.newSlotDetails}});
        }
...
async function approveScheduleProposal(proposal) {
    ...
    try {
        ...
        const operation = proposal.operation || (proposal.proposal && proposal.proposal.operation) || 'modify';
        const timeSlots = proposal.timeSlots || (proposal.proposal && proposal.proposal.timeSlots);
        const newSlotDetails = proposal.newSlotDetails || (proposal.proposal && proposal.proposal.newSlotDetails);
        const title = proposal.title || (proposal.proposal && proposal.proposal.title);
        const changes = proposal.changes || (proposal.proposal && proposal.proposal.changes);

        if (operation === 'delete_slots' && timeSlots && timeSlots.length > 0) {
            // 删除指定的时间段
        ...
        } else if (operation === 'add_slot' && newSlotDetails) {
            // 添加新时间段
            const newSlot = newSlotDetails;
        ...
        } else if ((operation === 'modify_slot' || operation === 'update') && timeSlots && timeSlots.length > 0 && newSlotDetails) {
            const targetSlot = timeSlots[0];
            const newSlot = newSlotDetails;

            const hasNewActivity = newSlot.activity && newSlot.activity.trim() !== '';
            const hasNewTime = newSlot.time && newSlot.time.trim() !== '';
            const hasNewDetail = newSlot.detail !== undefined;
            const hasNewIcon = newSlot.icon !== undefined;

            if (!(hasNewActivity || hasNewTime || hasNewDetail || hasNewIcon)) {
                console.warn('[Approve] modify_slot/update with empty newSlotDetails:', targetSlot, date);
            }
            ...
            existingSchedule.timeSlots = (existingSchedule.timeSlots || []).map(slot => {
                if (slot.activity === targetSlot || slot.time === targetSlot || slot.activity.includes(targetSlot)) {
                    return {
                        time: hasNewTime ? newSlot.time : slot.time,
                        activity: hasNewActivity ? newSlot.activity : slot.activity,
                        detail: hasNewDetail ? newSlot.detail : slot.detail,
                        icon: hasNewIcon ? newSlot.icon : slot.icon
                    };
                }
                return slot;
            });
            existingSchedule.highlights = `已修改: ${targetSlot} -> ${newSlot.activity || targetSlot}`;
        } else {
            // 默认修改
            if (title) {
                existingSchedule.title = title;
            ...
            if (changes && changes.length > 0) {
                existingSchedule.highlights = changes.join('; ');
            }
        }
