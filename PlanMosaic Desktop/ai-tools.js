        ...
        function: {
            ...
            parameters: {
                ...
                properties: {
                    operation: {
                        type: 'string',
                        enum: ['delete_slots', 'modify_slot', 'update', 'replace_slot', 'add_slot', 'modify_title', 'general_adjustment', 'delete_all_matching', 'update_all_matching', 'batch_delete_dates'],
                        description: '操作类型'
                    },
                    ...
                    timeSlots: { type: 'array', items: { type: 'string' }, description: '要操作的时间段或活动名称（delete_slots/modify_slot/replace_slot时必填）' },
                    newSlotDetails: {
                        type: 'object',
                        description: '新时间段的详细信息。modify_slot时只需提供要修改的字段（activity/time等）；replace_slot时必须提供完整新slot（time+activity必填）。',
                        properties: {
                            time: { type: 'string', description: '时间段，格式HH:MM-HH:MM' },
                            activity: { type: 'string', description: '活动名称（修改名称时必须提供）' },
                            detail: { type: 'string', description: '活动详情' },
