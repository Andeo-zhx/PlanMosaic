...
/**
 * 获取当前用户的数据目录。
 * @param {string} [username] - 可选用户名，用于按账号隔离数据
 * 如果已设置用户名或传入 username：rootDir/hashed_username/
 * 如果未登录：rootDir/（向后兼容）
 */
function getAppDataDir(username) {
    const rootDir = getAppDataRootDir();
    const user = username || activeUsername;
    if (user) {
        const hashed = hashUsername(user);
        let dataDir = path.join(rootDir, hashed);
 ...
 * @returns {string} 绝对路径
 */
function getDataPath(filename, username) {
    const dir = getAppDataDir(username);
...
/** 快捷方法 */
function getDataFilePath(username)   { return getDataPath('data.json', username); }
...
function getBackupDir(username) {
    try {
        const backupDir = getDataPath('backups', username);
...
// 清理旧备份
function cleanOldBackups(filePrefix, username) {
    try {
        const backupDir = getBackupDir(username);
        if (!backupDir) return;
        ...
        const toDelete = files.slice(MAX_COUNT);
        for (const f of toDelete) {
            try {
                fs.unlinkSync(f.path);
                console.log(`[Backup] Deleted old backup: ${f.name}`);
            } catch (e) {
                console.error(`[Backup] Failed to delete ${f.name}:`, e.message);
            }
        }

        if (totalSize > MAX_SIZE) {
            for (let i = toKeep.length - 1; i >= 0 && totalSize > MAX_SIZE; i--) {
                totalSize -= toKeep[i].size;
                try {
                    fs.unlinkSync(toKeep[i].path);
                    console.log(`[Backup] Deleted oversized backup: ${toKeep[i].name}`);
                } catch (e) {
                    console.error(`[Backup] Failed to delete ${toKeep[i].name}:`, e.message);
                }
            }
        }
    } catch (e) {
